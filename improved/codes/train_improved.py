...
class Trainer():
    ...
    def train(self):
        ...
        for epoch in range(self.epochs):
            ...
            for i, batch_data in enumerate(self.train_dataloader, 1):
                inputs_0 = batch_data[0].to(self.device)
                inputs_1 = batch_data[1].to(self.device)
                labels = batch_data[2].to(self.device)

                self.optimizer.zero_grad()
                outputs = self.model(inputs_0, inputs_1)
    ...
    def test(self):
        ...
        model.eval()
        with torch.no_grad():
            for i, batch_data in enumerate(self.test_dataloader, 1):
                inputs_0 = batch_data[0].to(self.device)
                inputs_1 = batch_data[1].to(self.device)
                labels = batch_data[2].to(self.device)
